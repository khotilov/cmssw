...
Due to its dual controllers, the SATABeast is best fed by two loggernodes,
...
\begin{enumerate}
  ...
  \item For each file written: database operations for file bookkeeping for the transfer system 
        and done by the SM executable
  \item Actual transfer process of files to Tier0 
        (and bookkeeping operations NOT done by the SM executable) 
\end{enumerate}
...
the DB access is entirely decoupled from the SM executable (see Section {\bf ???}).
...
\begin{figure}[t]
\begin{center}  
\includegraphics[width=0.9\textwidth]{Hardware/cruzetByNode.eps}
...
%
\subsection{Response Plans for Hardware Failure \label{sec:SMhardfail}}
...
but smaller annoying problems arise for the shift crew: 
disks filling up, crashing executives, corrupted files, 
bugs introduced in updated code or scripts,\ldots
