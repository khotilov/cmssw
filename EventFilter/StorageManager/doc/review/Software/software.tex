...
The approximately 640 Filter Farm nodes will be split into 8 separate
...
\begin{figure}[hbtp]
  \begin{center}
    \includegraphics[width=6.0in]{Software/sm_mess_structure.eps}
...
per luminosity section. Each SM instance uses a separate disk array (see the
hardware section).
...
outside of the SM application. The SM application writes a logfile that is read by a
PERL script running on the same node, and handles the database interaction
and Tier-0 notification.
A separate PERL script monitors the status of each of the closed streamer files via
the database and manages the removal of streamer files.

All scripts used in the Storage Manager are maintained by the Storage Manager group
...
\begin{figure}[hbtp]
  \begin{center}
    \includegraphics[width=4.5in]{Software/fm_example.eps}
